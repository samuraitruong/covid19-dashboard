...
const writeDataToClient = async (client, data, measurement, timestamp) => {
  logger.info(
    "Writing worldometer data to InfluxDB : measurement = %s",
    measurement
  );
...
const writeChartData = async (client, charts, country) => {
  ...
  for (const item of charts) {
    logger.info(
      "writing summary worldometer chart data : %s %s",
      item.key,
      country
    );
    const points = [];
    ...
    try {
      ...
      await client.writePoints(points);
      logger.info(
        "worldometer successful write chart data ... %s %s",
        item.key,
        country
      );
    } catch (err) {
      console.error(
        "worldometer Error writing chart data to influxDB %j %s",
        err.message || err,
        country,
        item.key
      );
...
const worldometerJob = async (client) => {
  try {
    ...
    // test me
    const globalData = await getDataFromUrl(
